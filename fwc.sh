...
apply_default_rules() {
    ...
    case "$OS" in
        ubuntu|debian)
            ...
            # 2. 修复：兼容旧版 UFW，手动添加 ICMP（Ping）规则到 UFW 底层文件
            local ICMP_RULE="-A ufw-before-input -p icmp --icmp-type echo-request -j ACCEPT"
            local RULE_FILE="/etc/ufw/before.rules"
            # 先检查规则是否已存在（避免重复添加）
            if ! grep -qF "$ICMP_RULE" "$RULE_FILE"; then
                echo "$ICMP_RULE" >> "$RULE_FILE"  # 追加规则到文件末尾
                echo "⚠️  已为旧版 UFW 添加 Ping 规则到 $RULE_FILE"
            fi
            ufw reload  # 重新加载规则，使修改生效
